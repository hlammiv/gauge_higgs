...
//
//   ./build/gh_string_pt <rep> <L> <ladder_list> <fixed> <mu2> <couplings|auto>
//                        [ntherm nmeas n_sweep nmd tau seed]
//     <rep>        = fund | adj | <Young rows e.g. 6 (=SU(2) spin-3)>[:real]
//   BETA axis (default; GH_PT_AXIS unset or =beta):
//     arg3 <beta_list> = the beta-ladder; arg4 <kappa> = the SHARED kappa.
//   KAPPA axis (GH_PT_AXIS=kappa):
//     arg3 <kappa_list> = the kappa-ladder; arg4 <beta> = the SHARED beta.
//     (Cures deep-kappa metastability: a kappa-ladder 0=ergodic pure SU(2) ...
//      up to deep Higgs routes ergodic small-kappa configs up the ladder.)
//     <list>       = comma list  1.0,1.4,1.7,2.0,2.5   OR  min:max:step  1.0:2.5:0.3
...
//
//   GH_PT_AXIS env: beta (default) | kappa -- selects the tempered coupling.
...
//                  (-> M independent single-(beta,kappa) runs, the merge-safe no-op).
...
// Parse a ladder string (beta OR kappa, identical grammar): "1.0,1.4,1.7" (comma
// list) OR "min:max:step" (inclusive of max within fp tolerance). Result is SORTED
// ascending; duplicates are removed and the strict-increase requirement is enforced
// by ReplicaTempering's ctor.
static std::vector<Real> parse_ladder(const std::string& s) {
  ...
  if (s.find(':') != std::string::npos) {
    ...
    if (parts.size() != 3) throw std::runtime_error("ladder range needs min:max:step");
    const Real lo = parts[0], hi = parts[1], step = parts[2];
    if (!(step > 0.0)) throw std::runtime_error("ladder range step must be > 0");
    for (Real b = lo; b <= hi + 1e-9 * std::max<Real>(1.0, std::abs(hi)); b += step)
      vals.push_back(b);
  } else {
    std::stringstream ss(s); std::string t;
    while (std::getline(ss, t, ',')) if (!t.empty()) vals.push_back(std::atof(t.c_str()));
  }
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::fprintf(stderr,
      "usage: %s <rep> <L> <ladder_list> <fixed> <mu2> <couplings|auto> "
      "[ntherm=80 nmeas=200 n_sweep=1 nmd=24 tau=1 seed=1]\n"
      "  GH_PT_AXIS=beta (default): arg3=<beta_list> ladder, arg4=<kappa> shared.\n"
      "  GH_PT_AXIS=kappa         : arg3=<kappa_list> ladder, arg4=<beta>  shared.\n"
      "  <list> = comma list (1.0,1.4,1.7,2.0,2.5) OR min:max:step (1.0:2.5:0.3)\n"
      "  run with just <rep> to print the channel C2 values and the required #couplings.\n", argv[0]);
    return 1;
  }
  ...
  // AXIS SELECT: GH_PT_AXIS=kappa -> kappa-ladder at shared beta; else beta-ladder
  // (default = original behavior).  arg3 is ALWAYS the ladder LIST; arg4 is the
  // single SHARED value of the OTHER coupling.
  bool axis_kappa = false;
  if (const char* e = std::getenv("GH_PT_AXIS")) {
    std::string v(e);
    std::transform(v.begin(), v.end(), v.begin(),
                   [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    if (v == "kappa") axis_kappa = true;
    else if (v != "" && v != "beta") {
      std::fprintf(stderr, "error: GH_PT_AXIS must be 'beta' or 'kappa', got '%s'\n", e); return 1;
    }
  }

  // arg 3 = ladder list (beta-ladder for beta-axis, kappa-ladder for kappa-axis).
  std::vector<Real> ladder;
  try { ladder = parse_ladder(argv[3]); }
  catch (const std::exception& e) {
    std::fprintf(stderr, "%s-list error: %s\n", axis_kappa ? "kappa" : "beta", e.what()); return 1;
  }
  if (ladder.size() < 2) {
    std::fprintf(stderr, "error: tempering needs >=2 %s rungs, got %zu (give a comma list or min:max:step)\n",
                 axis_kappa ? "kappa" : "beta", ladder.size());
    return 1;
  }
  // arg4 is the SHARED OTHER coupling: kappa (beta-axis) or beta (kappa-axis).
  const Real shared = argf(4, axis_kappa ? 2.3 : 0.2);
  const Real kappa  = axis_kappa ? 0.0 /*per-rung; set by ladder*/ : shared;  // shared kappa (beta-axis)
  const Real fixed_beta = axis_kappa ? shared : 0.0;                          // shared beta  (kappa-axis)
  ...
  // Build the tempered ladder. The ctor enforces strictly-increasing rungs (we sorted
  // + de-duped above) and seeds replica k with seed + k*seed_stride.
  //   beta-axis : beta-ladder = arg3, shared kappa = arg4 (set below).
  //   kappa-axis: kappa-ladder = arg3, shared beta  = arg4 (passed to the ctor).
  std::unique_ptr<ReplicaTempering<kDim, kN>> ptp;
  try {
    if (axis_kappa)
      ptp = std::make_unique<ReplicaTempering<kDim, kN>>(
          typename ReplicaTempering<kDim, kN>::KappaLadder{}, L, *rep, ladder, fixed_beta, seed);
    else
      ptp = std::make_unique<ReplicaTempering<kDim, kN>>(L, *rep, ladder, seed);
  }
  catch (const std::exception& e) { std::fprintf(stderr, "ladder error: %s\n", e.what()); return 1; }
  ReplicaTempering<kDim, kN>& pt = *ptp;

  // Shared couplings on every replica. On the beta-axis kappa is shared (set_kappa);
  // on the kappa-axis kappa IS the ladder (already set by the ctor) and beta is shared
  // (already set by the ctor), so we do NOT call set_kappa there.
  if (!axis_kappa) pt.set_kappa(kappa);
  ...
  if (!axis_kappa) {
    // BETA AXIS -- byte-identical to the original output.
    ...
    // KAPPA AXIS -- shared beta, kappa is the ladder.
    std::printf("# D=%d SU(%d) L=%d^%d (cubic)  beta=%.3f mu2=%.3f nmd=%d  potential=multi-invariant%s\n",
                kDim, kN, Lext, kDim, fixed_beta, mu2, nmd, frozen ? "  [FROZEN |phi|=1]" : "");
    std::printf("# TEMPERED kappa-ladder (%d rungs): ", M);
    for (int k = 0; k < M; ++k) std::printf("%s%.4g", k ? "," : "", pt.kappa(k));
    std::printf("\n");
  }
  ...
  for (int k = 0; k < M; ++k) {
    ...
    if (!axis_kappa)
      ...
      std::snprintf(label, sizeof label,
                    "########## rung %d/%d : kappa = %.4g  (hmc_acc=%.4f) ##########",
                    k, M, pt.kappa(k), pt.replica(k).acceptance());
  ...
  if (!pt.enabled) {
    std::printf("# swaps DISABLED (GH_TEMPER off): %d independent single-%s runs, no exchanges.\n",
                M, axis_kappa ? "kappa" : "beta");
  } else {
    ...
    if (!axis_kappa)
      ...
      std::printf("# %-6s %-14s %-14s %s\n", "pair", "kappa_k", "kappa_{k+1}", "accept_rate");
    bool connected = true;
    for (std::size_t p = 0; p < pt.n_pairs(); ++p) {
      ...
      std::printf("  (%zu,%zu)  %-14.4g %-14.4g %.4f%s\n",
                  p, p + 1, pt.ladder_value(p), pt.ladder_value(p + 1), rate,
