...
//   5. kappa=0 pure-gauge <plaquette> agrees with the validated U1HMC (same compact-U(1) measure).
#include "check.hpp"
...
// Total frozen action of a config (beta*A - kappa*B), for FD-style consistency checks.
template <int D>
static Real frozen_action(const U1Frozen<D>& s) {
  return s.beta * s.A() - s.kappa * s.B();
}
...
static void test_vonmises() {
  ...
  for (Real a : {0.5, 2.0, 5.0}) {
    ...
    double sc = 0, ss = 0;
    for (int i = 0; i < N; ++i) {
      const Real ang = draw_vonmises(a, rng, Rng::key(0x777, i));
      sc += std::cos(ang); ss += std::sin(ang);
    }
    const double mc = sc / N, ms = ss / N;
    const double ref = std::cyl_bessel_i(1, a) / std::cyl_bessel_i(0, a);   // mean resultant length
    char m[96];
    std::snprintf(m, sizeof m, "vonMises <cos> a=%.1f got %.4f exp %.4f", a, mc, ref);
    CHECK_CLOSE(mc, ref, 0.01, m);
    std::snprintf(m, sizeof m, "vonMises <sin> a=%.1f got %.4f exp 0", a, ms);
    CHECK_CLOSE(ms, 0.0, 0.01, m);
  }
}
  ...
  for (std::int64_t x = 0; x < s.lat.vol; ++x)
    for (int mu = 0; mu < D; ++mu)
      for (int m = 1; m < q; ++m) {
        ...
        s.th[idx] = o + 2.0 * kPi * m / q;
        worst = std::max(worst, std::fabs(s.B() - B0));
        s.th[idx] = o;
      }
  char msg[64]; std::snprintf(msg, sizeof msg, "Z_q hop dB=0 (q=%d)", q);
  CHECK_CLOSE(worst, 0.0, 1e-11, msg);
}
...
static void test_matter_local(int q) {
  ...
  for (std::int64_t x : {std::int64_t(0), std::int64_t(7), s.lat.vol - 1}) {
    ...
    // local prediction: S(chi) = const - a cos(chi - alpha)
    const Real pred = -a * (std::cos(s.chi[x] + delta - alpha) - std::cos(s.chi[x] - alpha));
    const Real S0 = frozen_action<D>(s);
    const Real old = s.chi[x]; s.chi[x] = old + delta;
    const Real glob = frozen_action<D>(s) - S0;
    s.chi[x] = old;
    worst = std::max(worst, std::fabs(pred - glob));
  }
  char msg[72]; std::snprintf(msg, sizeof msg, "matter resultant vs global dS (q=%d)", q);
...
static void test_gauge_local(int q) {
  ...
  const Real delta = 0.137;
  Real worst = 0.0;
  for (std::int64_t x : {std::int64_t(0), std::int64_t(9), s.lat.vol - 1})
    for (int mu = 0; mu < D; ++mu) {
      const std::size_t idx = x * D + mu;
      const Real psi = s.chi[s.lat.neighbor_fwd(x, mu)] - s.chi[x];
      const Real o = s.th[idx];
  ...
  char msg[72]; std::snprintf(msg, sizeof msg, "gauge local dS vs global (q=%d)", q);
  CHECK_CLOSE(worst, 0.0, 1e-9, msg);
}

// ---- 5. kappa=0: frozen pure-gauge <plaq> agrees with U1HMC (same compact-U(1) measure) ----
template <int D>
static void test_kappa0_vs_hmc() {
  const Real beta = 2.0;
  // frozen sampler, matter decoupled (kappa=0)
  U1Frozen<D> s(cube<D>(4), 101); s.beta = beta; s.kappa = 0.0; s.q = 2; s.zq_hop = false; s.hot(0.8);
  s.thermalize(300);
  double pf = 0; int nf = 0;
  for (int i = 0; i < 600; ++i) { s.sweep(); pf += s.avg_plaq(); ++nf; }
  pf /= nf;
  // HMC reference at kappa=0
  U1HMC<D> h(cube<D>(4), 202); h.beta = beta; h.kappa = 0.0; h.lambda = 0.5; h.q = 2; h.tau = 1.0; h.nmd = 20;
  h.hot(0.8); h.cold_phi(0.5);
  for (int i = 0; i < 200; ++i) h.trajectory();
  double ph = 0; int nh = 0;
  for (int i = 0; i < 600; ++i) { h.trajectory(); ph += avg_plaquette<D>(h.th, h.lat); ++nh; }
  ph /= nh;
  char msg[96]; std::snprintf(msg, sizeof msg, "kappa=0 <plaq> frozen %.4f vs HMC %.4f (beta=%.1f)", pf, ph, beta);
  CHECK_CLOSE(pf, ph, 0.02, msg);
}
